...
class RadiusClientUpdater {
    ...
    boolean updateFiles() throws IOException {
        ...
        RadiusClientPluginUtils.readAllLinesFromClasspath(classLoader.getResourceAsStream(updateList)).stream()
    ...
    private void inputStreamToFile(Map.Entry<String, String> pair) {
        try (InputStream inputStream = classLoader.getResourceAsStream(pair.getKey())) {
            if (inputStream != null)
                RadiusClientPluginUtils.copyInputStreamToFile(inputStream, new File(pair.getValue()));
        } catch (IOException e) {
    ...
    private boolean sameVersion() throws IOException {
        Path wikiFilePath = RadiusClientPluginUtils.toPluginWikiFilePath(rootPagePath, mainWikiFile);
        String filesystemVersion = RadiusClientPluginUtils.retrieveVersionFromWikiFile(wikiFilePath);
